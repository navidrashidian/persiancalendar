...
const int NON_LEAP_CORRECTION_SIZE = sizeof(NON_LEAP_CORRECTION) / sizeof(NON_LEAP_CORRECTION[0]);

bool is_non_leap_correction(int year) {
    int left = 0, right = NON_LEAP_CORRECTION_SIZE - 1;
    while (left <= right) {
        int mid = left + (right - left) / 2;
        if (NON_LEAP_CORRECTION[mid] == year) return true;
        if (NON_LEAP_CORRECTION[mid] < year) left = mid + 1;
        else right = mid - 1;
    }
    return false;
}
